        ...
        <div class="container">
                ...
                <ul class="actions">
                    ...
                    <li class="dropdown">
                        ...
                        <ul class="dropdown-menu dm-icon pull-right">
                            ...
                            <br/>
                            <li>
                                <nobr>&nbsp;&nbsp;<input type="text" name="customerManagementNo" id="masthead-label-2"class="abc input-default" placeholder="请输入领料出库单编号" value="">&nbsp;<button style="height:26px; width:26px" onClick="pickingOutboundDeliveryOrderNo_del()" class="btn btn-default btn-icon"><i style="font-size:x-large" class="zmdi zmdi-delete"></i></button></nobr>
                            </li>
            ...
            <div class="card">
                ...
                <table id="data-table-command" class="table table-striped table-vmiddle">
                    <thead>
                    <tr>
                        <th data-column-id="pickingOutboundDeliveryOrderNo" data-type=""data-identifier="true">领料出库单编号</th>
                        <th data-column-id="pickingOutboundDeliveryOrderNo"data-identifier="true">商品名称</th>
                        <th data-column-id="supplier" data-order=""data-identifier="true">供应商</th>
                        <th data-column-id="procurementStaff" data-order=""data-identifier="true">采购员工</th>
                        <th data-column-id="commodityOutboundDeliveryDate" data-order=""data-identifier="true">商品出库日期</th>
                        <th data-column-id="commodityOutboundDeliveryQuantity" data-order="desc"data-identifier="true">商品出库数量</th>
                        <%--<th data-column-id="commands" data-formatter="commands" data-sortable="false"data-identifier="true"data-row-id="commands">操作权限</th>--%>
                    </tr>
                    </thead>
                    <tbody>
                    <c:forEach items="${pickingOutboundList}" var="userList">
                        <tr>
                            <td><input  type="checkbox" name="id[]"
                                        value="${userList.pickingOutboundDeliveryOrderNo}" />${userList.pickingOutboundDeliveryOrderNo}</td>
                            <td>${userList.pickingOutboundDeliveryOrderNo}</td>
                            <td>${userList.supplier}</td>
                            <td>${userList.procurementStaff}</td>
                            <td><fmt:formatDate value="${userList.commodityOutboundDeliveryDate}"
                                                pattern="yyyy-MM-dd HH:mm:ss" /></td>
                            <td>${userList.commodityOutboundDeliveryQuantity}</td>
                        </tr>
                    </c:forEach>
                    </tbody>
                </table>
...
//              });
    ...
    function pickingOutboundDeliveryOrderNo_del(){
        ...
        if(r==true){
            $.get("/pickingOutboundController/pickingOutboundDelete.do?pickingOutboundNo="+id,function(data){
                if(data=="ok"){
                    alert("删除失败！");
                }else{
                    alert("删除成功！");
    ...
    $('#update_button').click(function () {
        //1、判断员工的正则表示
        $.ajax({
            url:'<%=basePath%>/pickingOutboundController/savePickingOutbound.do',
            type:"POST",
    ...
    $('#update_button2').click(function () {
        $.ajax({
            url:'<%=basePath%>/pickingOutboundController/addPickingOutbound.do',
            type:"POST",
